...
export function createTamboTools(callbacks: ToolBridgeCallbacks) {
  const executeGraphQLQueryTool = {
    name: "execute_graphql_query" as const,
    description:
      "Execute a read-only GraphQL query against the connected endpoint. Mutations and subscriptions are disabled. The query and results will be synced to the editor panels automatically. " +
  ...
  return [executeGraphQLQueryTool, getSchemaInfoTool] as [typeof executeGraphQLQueryTool, typeof getSchemaInfoTool];
}
